...
with open("names.csv", "wb") as my_csv_file:
   file_write = csv.writer(my_csv_file1, delimiter=',')
   file_write.writerow(["name","gender"])
...
def get_names(url,gender):
    ...
    if(i.dl):
        if(i.dl.dt):
            if(i.dl.dt.b):
                if(i.dl.dt.b.a):
                    with open("names.csv", "a") as my_csv_file:
                       file_write = csv.writer(my_csv_file, delimiter=',')
                       file_write.writerow([i.dl.dt.b.text, gender])
